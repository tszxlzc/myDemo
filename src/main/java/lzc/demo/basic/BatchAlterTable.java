...
//		alterTable();
	...
	private static void alterTable(){
		for(int i =0; i< 30; i++){
			...
			System.out.println("ADD COLUMN `listen_sub_question_id` bigint(20) DEFAULT 0 COMMENT '听力子题id';");
			System.out.println("-- 重建新的索引");
			System.out.println("ALTER TABLE tai_homework_student_detail_" + i);
			System.out.println("ADD UNIQUE KEY `uk_student_howework_quesiton` (`homework_id`,`student_id`,`biz_type`,`biz_id`,`listen_sub_question_id`) USING BTREE COMMENT '唯一索引：保证学生提交试题的唯一性';");
			System.out.println("");
		}
	}
	...
	private static void rollback(){
		for(int i = 0; i < 30; i++){
			...
			System.out.println("DROP COLUMN `listen_sub_question_id`;");
